...
const socket = { socketOptions: { connectTimeoutMS: 30000, keepAlive: 1, socketTimeout: 0 } }
const options = {
  server: socket,
  replset: socket,
...
function logging (db) {
  ...
  db.on('open', () =>
    logger.debug(`Database connection to ${dbName} opened.`)
  )
  db.on('disconnected', () => {
  ...
  db.on('error', err =>
    logger.error(
      `Database connection to ${dbName} error.`,
